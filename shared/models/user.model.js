...
export class User extends AbstractModel {
    ...
    isCaptain() {
        ...
        for (var i in this.roles) {
            let r = this.roles[i];
            if (r.level == Role.LEVELS.CAPTAIN)
                return true;
        }
    }
    isJournalist() {
        ...
        for (var i in this.roles) {
            let r = this.roles[i];
            if (r.level == Role.LEVELS.JOURNALIST)
                return true;
        }
    }
    isPhotographer() {
        ...
        for (var i in this.roles) {
            let r = this.roles[i];
            if (r.level == Role.LEVELS.PHOTOGRAPHER)
                return true;
        }
    }
    isStaff() {
    ...
    hasAccessToTeam(team) {
        ...
        for (var i in this.roles) {
            let r = this.roles[i];
            if (r.level == Role.LEVELS.CAPTAIN && r.team._id == team._id)
                return true;
        }
        return false;
    }
